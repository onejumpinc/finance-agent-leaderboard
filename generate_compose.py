...
PARTICIPANT_TEMPLATE = """  {name}:
    ...
    environment:{env}
    healthcheck:
      test: ["CMD", "python", "-c", "import urllib.request; urllib.request.urlopen('http://localhost:{port}/.well-known/agent-card.json', timeout=2).read()"]
      interval: 5s
      timeout: 3s
      retries: 10
      start_period: 30s
...
def resolve_image(agent: dict, name: str) -> None:
    ...
    if has_image:
        # Keep agentbeats_id in the generated A2A scenario for attribution,
        # while allowing a digest-pinned image when the registry record uses
        # an amber manifest and therefore has no legacy docker_image field.
